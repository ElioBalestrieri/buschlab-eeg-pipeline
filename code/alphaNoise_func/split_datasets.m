function split_datasets(cfg, EEG)
% alphaNoise specific
...
%% select component best representing alpha in the prestim win
prestim = [-1.5, 0];
[alphaComps, pow_spctr, avg_spctr] = local_selectAlphaComp(EEG, prestim);
DS_dipolefits = pop_multifit(EEG, 1:size(EEG.icaact, 1), 'dipplot', 'off');

% MNI from Juelich:
% https://atlases.ebrains.eu/viewer/
MNI_coords = [0, -89, 0];

dist_calc = nan(numel(alphaComps.idxs), 1); acc_comp = 0;

for idipole = alphaComps.idxs'

    acc_comp = acc_comp +1;
    coords_dip = DS_dipolefits.dipfit.model(idipole).posxyz;
    
    if isempty(coords_dip)
        coords_dip = [666, 666, 666];
    end

    diff_cords = coords_dip - MNI_coords;
    euc_dist = sqrt(diff_cords * diff_cords');
    dist_calc(acc_comp) = euc_dist;

end

% buld a matrix of: 1. original component indexes, 2. current indexes based
% on power and 3. distance from calcarine
[~, idxs_distance_from_calc] = sort(dist_calc, 'ascend');

srtd_dist_idxs = 1:length(idxs_distance_from_calc);
srtd_dist_idxs(idxs_distance_from_calc) = srtd_dist_idxs;

mat_fullcomps = [alphaComps.idxs, (1:numel(alphaComps.idxs))', srtd_dist_idxs'];
mat_fullcomps(:, end+1) = mat_fullcomps(:, 2) + mat_fullcomps(:, 3);

% finally sort components based on alpha power AND dipole position
% select the highest component
mat_fullcomps = sortrows(mat_fullcomps, 4);
theChosen = mat_fullcomps(1,1);

% create structure for component summary
chosen_IC = DS_dipolefits.dipfit.model(theChosen);


%% now that teh compoennt has been chosen...
% compute all the necessary analyses: 

ICAfield.full = [];
ICAfield.alpha = alphaComps;
...
chosen_IC.rescaled_topo = rescaled(:, theChosen);


% ICAfield alpha
ICAfield.alpha.idxs_comps_kept = theChosen;
ICAfield.topomaps_rescaled = rescaled(:, theChosen);
ICAfield.alpha.FFT.avg_spectra = avg_spctr(theChosen, :);
ICAfield.alpha.FFT.freqs = alphaComps.xfreq; 

mask_alpha = (alphaComps.xfreq>=min(alphaComps.alpharange)) & ...
             (alphaComps.xfreq<=max(alphaComps.alpharange));

singletrial_FFT_range = pow_spctr(theChosen, mask_alpha, :);
...
selected_comp_data = EEG.icaact(theChosen, :, :);
...
prestim_alphapow = ICAfield.alpha.FFT.singletrial;
...
logfile_table = baseline_corrected.trialinfo;

% save files
save(fullfile(outfolder, [subjname, '_mat_data.mat']), 'mat_data');
save(fullfile(outfolder, 'time_vect.mat'), 'time_vect');
save(fullfile(outfolder, 'prestim_alphapow.mat'), 'prestim_alphapow');
